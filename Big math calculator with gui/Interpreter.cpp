...
Interpreter::Interpreter()
{
	...
	stack<string> ops_s; //the stack of string operands
	...
	VTmath_status = false; //by default the calculator should be in normal mode.
...
void Interpreter::set_VTmath_status(bool stat) //Used to toggle if VTmath mode is active or not
{
	VTmath_status = stat;
}
bool Interpreter::get_VTmath_status() // Checks the if VTmath mode is active or not
{
	return VTmath_status;
}
...
bool Interpreter::is_NOT_op(string temp) //This function is used to determine if the given input is a valid number or not
{
	if(temp == "+" || temp == "-" || temp == "*" || temp == "/" || temp == "^" || temp == "p" || temp == "P" || temp == "d" ||
		temp == "r" || temp == "z" || temp == "Z" || temp == "n" || temp == "f")
	{
		return false;
	}
...
bool Interpreter::compute(istream & input, ofstream & output)
{
	...
	while(input >> temp)
	{
		...
		else if(VTmath_status == true)//VTmath can take care of negative input and leading zeros but we need this if we're placing the string directly into the stack in VTmath mode
		{
			if(temp.at(0) == '_')//If we recieve a negative number (has an underscore in front) as input we should handle it correctly before doing anything further
			{
		...
		if(temp == "*") //check for multiplication operation only if there are at least 2 operands left in the stack
		{
			...
			else if(VTmath_status == true)
			{
				...
				ops_s.push(RESULT.vec_to_string());
		...
		{
			//cout<<"I'm about to do addition" <<endl;
			match = true;
		...
		if(temp == "r") //If the user wants to reverse the top 2 values on the top of the stack
		{
			...
			{
				string top = ops_s.top();
				ops_s.pop();
				string bottom = ops_s.top();
				ops_s.pop();
		...
		if(temp == "f") //The printing the whole the stack should only be done if the stack has contents in it.
		{
			...
			{
				stack<string> ops_s_temp;
				ops_s_temp = ops_s;
				
				while(ops_s_temp.empty() == false)
				{
					
					output<< ops_s_temp.top() <<"\r";
					ops_s_temp.pop();
		...
		if(temp == "Z")//The user wants to count the number of digits in the value on the top of the stack
		{
			...
			else if(VTmath_status == true)
			{
				...
				ops_s.push(std::to_string(s.length()));
			}
		...
		else if (match == false)
		{
			...
			status = false;
			//If there is an invalid operand or command, we return the status as false which will mean EXIT_FAILURE
			return status;
		}
