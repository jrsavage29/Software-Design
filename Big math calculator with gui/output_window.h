...
public:
	OutputWindow(QWidget* parent = nullptr);	
	 ...
	 void reset_stack(); //clear the stack
	 bool Is_VTmath_Active();//Used to check the VTmath status variable in the interpreter class
	 void set_VTmath_mode(bool stat); //Used to set the status of the VTmath status variable in the interpreter class.
